  ...
  return (
    <div className={style(overalStyle)}>
      <button className={style('button', buttonStyle)} onClick={onClickHandler}>
        {name === 'header' ? (
          <span className={style('button_text', textStyle)}>
            <span className={style('color_span')}>D</span>ora <span className={style('color_span')}>N</span>ails
          </span>
        ) : (
          <span className={style('button_text', textStyle)}>{name}</span>
        )}
      </button>
    </div>
  );
