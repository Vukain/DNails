...
const SERICES_DATA = {
  'typ usługi 1': [
    { serviceName: 'Usługa 1', servicePrice: 'XYZ zł' },
    { serviceName: 'Usługa 2', servicePrice: 'XYZ zł' },
    { serviceName: 'Usługa 3', servicePrice: 'XYZ zł' },
  ],
  'typ usługi 2': [
    { serviceName: 'Usługa 1', servicePrice: 'XYZ zł' },
    { serviceName: 'Usługa 2', servicePrice: 'XYZ zł' },
...
export const Prices = () => {
    ...
    const services = Object.keys(SERICES_DATA).map((group, idx) => (
      <div className={style('group')} key={idx}>
        ...
        <ul className={style('items')}>
          {SERICES_DATA[group].map(({ serviceName, servicePrice }, idx) => (
            <li className={style('item')} key={idx}>
              <div className={style('service')}>{serviceName}</div>
              <div className={style('separator')}></div>
              <div className={style('price')}>{servicePrice}</div>
            </li>
          ))}
        </ul>
      </div>
  ...
  return (
    <div className={style()}>
      ...
      <div className={style('services')}>{servicesList}</div>
    </div>
  );
};
