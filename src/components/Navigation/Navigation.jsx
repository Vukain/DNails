...
export const Navigation = () => {
  ...
  const sectionScroller = useCallback(
    (idx, level) => {
  ...
  const buttonsData = [
    { name: 'header', position: 'first', section: 0, level: 1 },
    { name: 'usługi', position: 'second', section: 1, level: 2 },
    { name: 'kolory', position: 'third', section: 2, level: 2 },
    { name: 'wizyty', position: 'fourth', section: 3, level: 2 },
    { name: 'galeria', position: 'fifth', section: 4, level: 2 },
  ];
  const buttonsList = buttonsData.map((item, idx) => (
    <NavigationButton
      scroller={sectionScroller}
      hiddenBurger={mobileNavigationHidden}
      hideNavigation={setMobileNavigationHidden}
      name={item.name}
  ...
  return (
    <nav className={style({ lower_level: currentLevel === 2, hidden: mobileNavigationHidden })}>
      {buttonsList}
      <div className={style('hamburger', { active: !mobileNavigationHidden })} onClick={hamburgerHandler}>
        {' '}
        <div className={style('hamburger_icon')}></div>
      </div>
    </nav>
  );
};
