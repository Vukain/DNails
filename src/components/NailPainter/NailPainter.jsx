...
export const NailPainter = () => {
  ...
  const colorSquares = filteredColors.map((element, idx) => (
    <div
      key={idx}
      className={style('color_select', { selected: element.color === lacquerColor })}
      style={{ backgroundColor: element.color }}
      onClick={() => colorChanger(element.color)}
    ></div>
  ));
  ...
  const paintListener = (e, mode) => {
    if (isPainting) {
      ...
      if (mode === 'touch') {
        canvasContext.arc(
          e.targetTouches[0].clientX - cursorX,
          e.targetTouches[0].clientY - cursorY,
          16,
          0,
          2 * Math.PI,
          false,
    ...
    <section className={style()}>
      <div className={style('nail_canvas', { hidden: currentMobileSection === 0 })}>
        ...
        <div className={style('canvas_wrapper')} ref={wrapperRef}>
          ...
          <div
            ...
            onTouchStart={(e) => onMouseDownHandler(e, 'touch')}
            onMouseMove={paintListener}
            onTouchMove={(e) => paintListener(e, 'touch')}
      ...
      <div className={style('color_picker', { hidden: currentMobileSection === 1 })}>
        ...
        <div className={style('color_filter')}>
          <div
            className={style('color_filter_option', { active: lacquerType === 'standard' })}
            onClick={() => setLacquerType('standard')}
          >
            <h3 className={style('option_text')}>STANDARD</h3>
          </div>
          <div
            className={style('color_filter_option', { active: lacquerType === 'metallic' })}
            onClick={() => setLacquerType('metallic')}
          >
            <h3 className={style('option_text')}>METALICZNE</h3>
          </div>
          <div
            className={style('color_filter_option', { active: lacquerType === 'pearl' })}
            onClick={() => setLacquerType('pearl')}
          >
            <h3 className={style('option_text')}>PERŁOWE</h3>
          </div>
        </div>
        <div className={style('color_selector')}>{colorSquares}</div>
      </div>

      <ContentSwitcher
        options={['kolory', 'tester']}
