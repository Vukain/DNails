...
export const AppointmentForm = () => {
  ...
  const today = moment();
  const todayDate = `${today.year()}-${today.month() > 8 ? today.month() + 1 : '0' + (today.month() + 1)}-${
    today.date() > 9 ? today.date() : '0' + today.date()
  }T08:30`;

  useEffect(() => {
    setFormValidity(
      [nameInputValidity, surnameInputValidity, emailInputValidity, dateInputValidity, messageInputValidity].every(
        (bool) => bool,
      ),
    );
    ...
    <div className={style()}>
      <form className={style('form')} action="submit">
        <div className={style('title')}>
          <h2 className={style('title_text')}>UMÓW WIZYTĘ</h2>
        </div>
        <input
          className={style('input', { invalid: !nameInputValidity && touched })}
          aria-label="imię"
          type="text"
          placeholder="IMIĘ"
          name="imie"
          ref={nameInputRef}
          onBlur={() => onBlurHandler(nameInputRef, setNameInputValidity)}
          onClick={() => {
            setTouched(true);
          }}
        />
        <input
          className={style('input', { invalid: !surnameInputValidity && touched })}
          aria-label="nazwisko"
          type="text"
          placeholder="NAZWISKO"
          name="nazwisko"
          ref={surnameInputRef}
          onBlur={() => onBlurHandler(surnameInputRef, setSurnameInputValidity)}
          onClick={() => {
            setTouched(true);
          }}
        />
        <input
          className={style('input', { invalid: !emailInputValidity && touched })}
          aria-label="email"
          type="email"
          placeholder="EMAIL"
          name="email"
          ref={emailInputRef}
          onBlur={() => onBlurHandler(emailInputRef, setEmailInputValidity, 'email')}
          onClick={() => {
            setTouched(true);
          }}
        />
        <input
          className={style('input', { invalid: !dateInputValidity && touched })}
          aria-label="data"
          type="datetime-local"
          placeholder="DATA"
          name="data"
          ref={dateInputRef}
          onBlur={() => onBlurHandler(dateInputRef, setDateInputValidity)}
          onClick={() => {
            setTouched(true);
          }}
        ...
        />
        <textarea
          className={style('message', { invalid: !messageInputValidity && touched })}
          aria-label="wiadomość"
          name="message"
          cols="30"
          rows="10"
          ref={messageInputRef}
          onBlur={() => onBlurHandler(messageInputRef, setMessageInputValidity)}
          onClick={() => {
            setTouched(true);
          }}
        ></textarea>
