...
export const Calendar = () => {
  ...
  useEffect(() => {
    ...
    const emptyDaysCurrentMapped = Array.from(Array(startDayCurrent > 1 ? startDayCurrent - 1 : 6), (_, i) => (
      <div key={'empty' + i} className={style('day', { not_empty: false })}></div>
    ));
    const emptyDaysNextMapped = Array.from(Array(startDayNext > 1 ? startDayNext - 1 : 6), (_, i) => (
      <div key={'empty' + i} className={style('day', { not_empty: false })}></div>
    ));
  ...
  useEffect(() => {
    const daysCurrentMapped = DUMMY_DAYS_CURRENT.current.map((day, idx) => (
      <div
        key={idx}
        onClick={() => selectDayHandler(idx, 'this')}
        className={style('day', {
          not_empty: true,
          selected: idx + 1 === selectedDay,
          passed: idx + 1 < moment().date(),
          busy: day['hours'].length > 1,
          unavailable: day['hours'].length > 3,
        })}
      >
        {' '}
        {idx + 1}{' '}
    ...
    const daysNextMapped = DUMMY_DAYS_NEXT.current.map((day, idx) => (
      <div
        key={idx}
        onClick={() => selectDayHandler(idx)}
        className={style('day', {
          not_empty: true,
          selected: idx + 1 === selectedDay,
          busy: day['hours'].length > 1,
  ...
  const hours = [8, 10, 12, 14, 16, 18].map((hour, idx) => (
    <div
      key={idx}
      className={style('hour', {
        unavailable:
          activeMonth === 0
            ? DUMMY_DAYS_CURRENT.current.length > 0
              ? DUMMY_DAYS_CURRENT.current[selectedDay - 1]['hours'].includes(hour)
              : false
            : DUMMY_DAYS_CURRENT.current.length > 0
            ? DUMMY_DAYS_NEXT.current[selectedDay - 1]['hours'].includes(hour)
            : false,
      })}
    >
      {hour}
  ...
  const weekDays = ['PON', 'WTO', 'ŚRO', 'CZW', 'PIĄ', 'SOB', 'NIE'].map((day, idx) => (
    <div key={idx} className={style('weekday')}>
      {day}
    </div>
  ));
    ...
    <div className={style()}>
      <div className={style('month')}>
        <div
          ...
          className={style('chosen_month', { inactive_month: activeMonth !== 0 })}
        >
        ...
        <div
          ...
          className={style('chosen_month', { inactive_month: activeMonth !== 1 })}
        >
          {months[nextMonth.month()]}
        </div>
      </div>

      <div className={style('week_labels')}>{weekDays}</div>
